    ...
    public class DataService
    {
        public static async Task<Tempo> GetPrevisaoDoTempo(string cidade)
        {
            ...
            if (resultado["weather"] != null)
            {
                ...
                {
                    Title = (string)resultado["name"],
                    Temperature = (string)(resultado["main"]["temp"]) + " C",
                    Wind = (string)resultado["wind"]["speed"] + " mph",
                    Humidity = (string)resultado["main"]["humidity"] + " %",
                    Visibility = (string)resultado["weather"][0]["main"]
                };
        ...
        public static async Task<Tempo> GetPrevisaoDoTempoGeoLocation(string lat, string longi)
        {
            ...
            if (resultado["weather"] != null)
            {
                ...
                double temperaturaCelcius = Math.Round((Convert.ToDouble(resultado["main"]["temp"]) - 273.15), 0);
                double velocidadeVento = Math.Round((Convert.ToDouble(resultado["wind"]["speed"]) * 1.609344), 0);
